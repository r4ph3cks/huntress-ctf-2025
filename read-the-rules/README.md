# Huntress CTF 2025 - ℹ️ Read The Rules Writeup
- **Team:** r4ph3cks
- **Date:** 01/10/2025

## Challenge Information

- **Category:** ℹ️ Information
- **Challenge Name:** ℹ️ Read The Rules
- **Description:** 
```
Please follow the rules for the Huntress CTF!
Read the rules at https://ctf.huntress.com/rules
If you look closely, you can find a flag!
```
- **Author:** [John Hammond](https://www.youtube.com/@_JohnHammond)
- **Points:** 10

## TL;DR

This challenge was pretty straightforward. Opening the developer tools in the browser revealed the flag hidden in a comment within the HTML source code.

![Flag in HTML Comment](images/flag_in_html_comment.png)

---

## Flag

```
flag{bf61aced6e7f9335385a70f33b20d188}
```

---

## Notes

- **Difficulty:** Easy
- **Time taken:** 1 minute